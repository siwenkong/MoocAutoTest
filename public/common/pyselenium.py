...
class PySelenium(object):
    ...
    def __init__(self, browser='ff', remoteAddress = None):
        ...
        if remoteAddress is None:
            if browser == "firefox" or browser == "ff":
                ...
                dr = webdriver.Opera()
            elif browser == "plantomjs":
                dr = webdriver.PhantomJS()
            elif browser == "edge":
                dr = webdriver.Edge()
        else:
            if browser == "RChrome":
                dr = webdriver.Remote(command_executor='http://' + remoteAddress + '/wd/hub', desired_capabilities=dc)
            elif browser == "RIE":
        ...
        try:
            ...
            raise NameError("Not found {0} browser,You can enter 'ie','ff','chrome','RChrome','RIe' or 'RFirefox'.".format(browser))
    ...
    def element_wait(self, css, secs=5):
        ...
        message = 'Element:{0} not found in {1} seconds.'.format(css, secs)

        if by == "id":
            WebDriverWait(self.driver, secs, 1).until(EC.presence_of_element_located((By.ID, value)), message)
        elif by == "name":
            WebDriverWait(self.driver, secs, 1).until(EC.presence_of_element_located((By.NAME, value)), message)
        elif by == "class":
            WebDriverWait(self.driver, secs, 1).until(EC.presence_of_element_located((By.CLASS_NAME, value), message))
        elif by == "link_text":
            WebDriverWait(self.driver, secs, 1).until(EC.presence_of_element_located(By.LINK_TEXT, value), message)
        elif by == "css":
            WebDriverWait(self.driver, secs, 1).until(EC.presence_of_element_located(By.CSS_SELECTOR, value), message)
        else:
            raise NameError("Please enter the correct targeting element,'id','name','class','link_text','xpath','css'.")
    ...
    def type(self, css, text):
        ...
        try:
            ...
            el.send_keys(text)
            self.my_print("{0} Typed element:<{1}> content: {2},Spend {3} seconds".format(success, css, text,
                                                                                          time.time() - t1))
        except Exception:
            self.my_print("{0} Unable to type element: <{1}> content: {2}, Spend {3} seconds".format(fail, css, text,
                                                                                                     time.time() - t1))
            raise
